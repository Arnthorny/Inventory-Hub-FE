...
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query" // <--- 1. The Imports
...
export default function InventoryPage() {
  ...
  const queryClient = useQueryClient() // <--- 2. Access the cache
  const [showForm, setShowForm] = useState(false)
  const [currentPage, setCurrentPage] = useState(1)

  // --- QUERY 1: FETCH USER PROFILE ---
  // Replaces: The auth check and user fetch in useEffect
  ...
  // --- QUERY 2: FETCH ITEMS ---
  // Replaces: The items fetch in useEffect
  // Benefit: This runs in parallel with the user fetch!
            ...
            <CardContent>
              ...
              >
                ...
                {createItemMutation.isError && (
                  <p className="text-red-500 text-sm">Failed to create item. Try again.</p>
                )}
              </form>
            </CardContent>
          </Card>
