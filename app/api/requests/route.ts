...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const item of body.items || []) {
      const { data: itemData } = await supabase.from("items").select("level").eq("id", item.item_id).single()
    ...
    console.log("[v0] Request created successfully:", createdRequest.id, "Auto-approved:", shouldAutoApprove)
