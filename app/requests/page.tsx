...
export default function RequestsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        console.log("[v0] Items loaded:", itemsData.items?.length || 0, "Error:", itemsData.error)
        setItems(itemsData.items || [])

        console.log("[v0] Fetching requests from API...")
        const requestsResponse = await fetch("/api/requests/list")
        const requestsData = await requestsResponse.json()
        console.log("[v0] Requests loaded:", requestsData.requests?.length || 0, "Error:", requestsData.error)
  ...
  const userRequests =
    user.role === "admin" || user.role === "staff" ? requests : requests.filter((r) => r.user_id === user.id)
        ...
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-muted-foreground mt-2">
              {isStaff ? "Manage item requests" : "Create and track your requests"}
            </p>
          </div>
          {!isStaff && (
            <Button onClick={() => setShowForm(!showForm)}>{showForm ? "View Requests" : "New Request"}</Button>
          )}
        </div>

        {showForm && !isStaff && items.length > 0 && <RequestForm items={items} onSubmit={handleCreateRequest} />}
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {userRequests.length === 0 ? (
            <Card className="col-span-full">
              <CardContent className="text-center py-8 text-muted-foreground">No requests found</CardContent>
            </Card>
          ) : (
